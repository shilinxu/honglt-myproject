...
#ifndef ARRAY_TEMPLATE_H__INCLUDED_
#define ARRAY_TEMPLATE_H__INCLUDED_
...
INT_PTR CArrayTemplate<TYPE,ARG_TYPE>::Append(const CArrayTemplate & Src)
{
	...
	ASSERT_VALID(this);
	ASSERT(this!=&Src);
	if (this==&Src) AfxThrowUserException();
...
void CArrayTemplate<TYPE,ARG_TYPE>::SetSize(INT_PTR nNewSize)
{
	...
	ASSERT_VALID(this);
	ASSERT(nNewSize>=0);
	if (nNewSize<0)	AfxThrowUserException();
...
void CArrayTemplate<TYPE,ARG_TYPE>::SetAtGrow(INT_PTR nIndex, ARG_TYPE newElement)
{
	...
	ASSERT_VALID(this);
	ASSERT(nIndex>=0);
	if (nIndex<0) AfxThrowUserException();
...
void CArrayTemplate<TYPE,ARG_TYPE>::InsertAt(INT_PTR nIndex, const CArrayTemplate & Src)
{
	...
	if (nStartIndex<0) AfxThrowUserException();
...
void CArrayTemplate<TYPE,ARG_TYPE>::InsertAt(INT_PTR nIndex, ARG_TYPE newElement, INT_PTR nCount)
{
	...
	ASSERT(nIndex>=0);
	ASSERT(nCount>0);
	if ((nIndex<0)||(nCount<=0)) AfxThrowUserException();
...
void CArrayTemplate<TYPE,ARG_TYPE>::RemoveAt(INT_PTR nIndex, INT_PTR nCount)
{
	...
	ASSERT(nCount>=0);
	ASSERT(nIndex+nCount<=m_nElementCount);
	if ((nIndex<0)||(nCount<0)||((nIndex+nCount>m_nElementCount))) AfxThrowUserException();
...
#endif // ARRAY_TEMPLATE_H__INCLUDED_