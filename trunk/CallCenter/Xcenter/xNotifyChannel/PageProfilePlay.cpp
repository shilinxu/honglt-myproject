...
BEGIN_MESSAGE_MAP(CPlayProfilePage, CSettingsPage)
	...
	ON_BN_CLICKED(IDC_MORE, OnChatRoom)
...
void CPlayProfilePage::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_MORE, m_wndMore);
...
void CPlayProfilePage::OnChatRoom()
{
	HINSTANCE hPrevInst = AfxGetResourceHandle();
	if ( m_hInstance ) AfxSetResourceHandle( m_hInstance );
	
	CChatPropertiesDlg dlg( m_pXMLChat, this );
	dlg.DoModal();
