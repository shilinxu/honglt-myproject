...
int CCallerChannel::bkIdle_init(EVTBLK *evtblk)
{
	...
	// debug
	int nChStateBefor = SsmGetChState( nChan );
	int nVoltageBefor = SsmGetLineVoltage( nChan );
	
	CChannel::StopChan();
	
	theApp.Message(MSG_ERROR, "debug[%i]: {bkIdle_init} ChState={%i -> %i}, Voltage={%i -> %i}", nChan,
		nChStateBefor,SsmGetChState(nChan), nVoltageBefor,SsmGetLineVoltage(nChan));
	//
...
struct state* CCallerChannel::FindUser_cmplt(EVTBLK *evtblk)
{
	...
	if ( evtblk->wEventCode == E_SYS_TIMEOUT )
	{
		...
		SetTimer( 1 * 1000 ); return &FindUser;
	}
