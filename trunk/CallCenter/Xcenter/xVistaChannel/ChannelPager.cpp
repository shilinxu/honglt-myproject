...
CChannel* CPagerChannel::ConvertFrom(CChannel* pParent, _RecordsetPtr xRecordset)
{
	...
	if ( ! pChannel->LoadFrom( xRecordset ) )
	{
		delete pChannel; return NULL;
	}
...
int CPagerChannel::RcvFnOper_init(EVTBLK *evtblk)
{
	...
	if ( nDtmfKey == '1' )	// 回复
	{
		typedef CChannel* (* PCTOR) (CChannel*, LPCTSTR, BOOL);
		PCTOR pCtor = (PCTOR)CPlayChannel::ConvertPager;
		
		CChannel* pChannel = pCtor( this, FeeNumber, TRUE );
		return (pChannel->*(pChannel->fcnp)->initiate)(evtblk);
	}
	else
	if ( nDtmfKey == '2' )	// 转发
	{
		...
		int nPos = xUsrFile.Find( ';' );
		if ( nPos > 0 ) xUsrFile = xUsrFile.Left(nPos);
		
		CChannel* pChannel = pCtor( this, (LPCTSTR)xUsrFile );
		return (pChannel->*(pChannel->fcnp)->initiate)(evtblk);
	}
