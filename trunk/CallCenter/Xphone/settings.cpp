...
void CSettings::Setup()
{
	...
	Add( _T("Network.MaxRate"), &Network.MaxRate, 1.0 / 30.0 );
	Add( _T("Network.MaxPacket"), &Network.MaxPacket, (1024 * 64) - 1 );
	
	Add( _T("Multicast.Host"), &Multicast.Host, "224.0.114.1" );
	Add( _T("Multicast.Port"), &Multicast.Port, 11800 );
	
	Add( _T("Audio.Input"), &Audio.Input, NULL );
	Add( _T("Audio.Output"), &Audio.Output, NULL );
