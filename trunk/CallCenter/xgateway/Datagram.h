...
public:
	virtual HRESULT	PostBuffer(BYTE* pBuffer, DWORD nLength, BYTE nType, DWORD nTimestamp, LPVOID lv);
...
struct RTPPayload : G3_UDP_HEADER
{
	...
	inline void SetAttribute(DWORD nLinkId, DWORD nTimestamp)
	{
		for ( RTPPayload* pPayload = this; pPayload; pPayload = pPayload->PayloadNext )
		{
			pPayload->nLinkId = SWAP_LONG(nLinkId);
			pPayload->nTimestamp	= SWAP_LONG(nTimestamp);
		}
	}
};
