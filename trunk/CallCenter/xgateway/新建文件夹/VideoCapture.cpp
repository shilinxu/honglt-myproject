...
BOOL CVideoCapture::CreateFilter(REFCLSID rclsid, LPCWSTR lpszName)
{
	CComPtr<IBaseFilter> pCapFilter;
	if ( pCapFilter.CoCreateInstance( rclsid ) != S_OK ) return FALSE;
	
	return InstallFilter( pCapFilter, lpszName );
}
...
BOOL CVideoCapture::InstallFilter(LPCWSTR xDisplayName, LPCWSTR lpszName)
{
	CComPtr<IBaseFilter> pCapFilter;
...
BOOL CVideoCapture::ConnectFilter(IBaseFilter* pRndFilter, IBaseFilter* pSrcFilter)
{
	Stop();
	
	CComPtr<IPin> pPinOut = NULL;
	m_pCapGraphBuilder->FindPin( pRndFilter, PINDIR_OUTPUT, NULL, NULL, TRUE, 0, &pPinOut );
	
	CComPtr<IPin> pPinIn = NULL;
	m_pCapGraphBuilder->FindPin( pSrcFilter, PINDIR_INPUT, NULL, NULL, TRUE, 0, &pPinIn );
...
BOOL CVideoCapture::GetCapFilter(LPCWSTR xDisplayName, IBaseFilter** pCapFilter)
{
	DWORD dwEaten;
