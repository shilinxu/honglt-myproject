...
void CTaskPanel::ClearBoxes()
{
	for ( POSITION posNext = GetBoxIterator() ; posNext ; )
	{
		GetNextBox( posNext )->DestroyWindow();
	}
...
void CTaskPanel::SetFooter(HBITMAP hBitmap, BOOL bDefault)
{
	...
	if ( bDefault && m_crPanelBack == RGB( 122, 161, 230 ) )
...
void CTaskPanel::OnSize(UINT nType, int cx, int cy) 
{
	...
	if ( m_pStretch != NULL && m_pStretch->GetOuterHeight() ) OnChanged();
}
...
void CTaskPanel::OnPaint() 
{
	...
	{
		dc.FillSolidRect( &rc, m_crPanelBack );
	}
}
...
void CTaskPanel::Layout(CRect& rcClient)
{
	...
	if ( m_pStretch && m_pStretch->GetOuterHeight() )
	{
		for ( POSITION pos = GetBoxIterator() ; pos ; )
		{
			...
			if ( pBox->IsWindowVisible() && pBox->m_nHeight && pBox != m_pStretch )
			{
...
BOOL CTaskBox::Create(CTaskPanel* pPanel, int nHeight, LPCTSTR pszCaption, UINT nIDIcon)
{
	...
	return CreateEx(pPanel, rect, pszCaption, nIDIcon);
}

BOOL CTaskBox::CreateEx(CWnd* pParentWnd, const RECT& rect, LPCTSTR pszCaption, UINT nIDIcon)
{
	if ( ! CWnd::Create( NULL, pszCaption, WS_CHILD|WS_VISIBLE|WS_CLIPCHILDREN|WS_CLIPSIBLINGS,
...
void CTaskBox::SetCaptionmark(HBITMAP hBitmap, BOOL bDefault)
{
	...
	if ( bDefault && m_bPrimary && CoolInterface.m_crTaskBoxPrimaryBack == RGB( 30, 87, 199 ) )
...
{
	if ( ! IsWindowVisible() || ! m_nHeight ) return 0;
...
void CTaskBox::PaintBorders()
{
	...
	if ( m_pPanel && m_pPanel->m_nCurve != 0 && m_bCaptionCurve )
	{
		dc.SetPixel( 0, 0, m_pPanel->m_crPanelBack );
		dc.SetPixel( 1, 0, m_pPanel->m_crPanelBack );
		dc.SetPixel( 0, 1, m_pPanel->m_crPanelBack );
		dc.SetPixel( rc.right - 1, 0, m_pPanel->m_crPanelBack );
		dc.SetPixel( rc.right - 2, 0, m_pPanel->m_crPanelBack );
		dc.SetPixel( rc.right - 1, 1, m_pPanel->m_crPanelBack );
	...
	{
		rc.top = rcc.bottom - 1;
		dc.Draw3dRect( &rc, CoolInterface.m_crTaskBoxCaptionBack, CoolInterface.m_crTaskBoxCaptionBack );
	}
}
