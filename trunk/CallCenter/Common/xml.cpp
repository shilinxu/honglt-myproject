...
void CXMLNode::ValueToString(LPCTSTR pszValue, CString& strXML)
{
	...
	LPTSTR pszOut	= strXML.GetBuffer( nLen + nLength ) + nLen;
	WideCharToMultiByte( CP_UTF8, 0, pszValue, nOut, pszOut, nLength, NULL, NULL );
	strXML.ReleaseBuffer( nLen + nLength );
#else
	...
	LPTSTR pszOut	= strXML.GetBuffer( nLen + nByte ) + nLen;
	nByte = WideCharToMultiByte( CP_UTF8, 0, pszWide, nLength, pszOut, nByte, NULL, NULL );
//	pszByte[ nByte ] = 0;

	strXML.ReleaseBuffer( nLen + nByte );
	delete [] pszWide;
#endif
}
#endif
