...
void CMD5Encrypt::EncryptData(LPCTSTR pszSrcData, TCHAR szMD5Result[16])
{
	...
	szMD5Result[0]=0;
#if _MSC_VER >= 1400
	for (int i=0;i<16;i++) _stprintf_s(&szMD5Result[i*2],16,"%02x",szResult[i]);
#else
...
#if _MSC_VER >= 1400
		_sntprintf_s(pszEncrypData+i*4,5,5,TEXT("%02x%02x"),szRandKey[i%ENCRYPT_KEY_LEN],bTempCode);
#else
		_sntprintf(pszEncrypData+i*4,5,TEXT("%02x%02x"),szRandKey[i%ENCRYPT_KEY_LEN],bTempCode);
#endif
	}
