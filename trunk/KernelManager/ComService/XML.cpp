...
BOOL CXMLNode::ParseIdentifier(LPCTSTR& pszBase, CString& strIdentifier)
{
	...
	pszBase += nParse;
#if _MSC_VER >= 1400
	_tcsncpy_s( strIdentifier.GetBuffer( nIdentifier ), nIdentifier, pszBase, nIdentifier );
#else
...
void CXMLNode::ValueToUniform(LPCTSTR pszValue, CString& strXML)
{
	...
	for ( ; *pszValue ; pszValue++ )
	{
		...
		case '\'':
			...
			break;
		default:
			if ( nChar > 127 && !IsDBCSLeadByte(nChar))
			{
				CString strItem;
...
#ifdef _UNICODE
	int nLength		= WideCharToMultiByte( CP_UTF8, 0, pszValue, nOut, NULL, 0, NULL, NULL );
	LPTSTR pszOut	= strValue.GetBuffer( nLen + nLength ) + nLen;
	WideCharToMultiByte( CP_UTF8, 0, pszValue, nOut, pszOut, nLength, NULL, NULL );
	strValue.ReleaseBuffer( nLen + nLength );
#else
	int nLength		= MultiByteToWideChar( CP_ACP, 0, pszValue, nOut, NULL, 0 );
	LPWSTR pszWide	= new WCHAR[ nLength + 1 ];
	nLength = MultiByteToWideChar( CP_ACP, 0, pszValue, nOut, pszWide, nLength );
	pszWide[ nLength ] = 0;
	
	int nByte		= WideCharToMultiByte( CP_UTF8, 0, pszWide, nLength, NULL, 0, NULL, NULL );
	LPTSTR pszOut	= strValue.GetBuffer( nLen + nByte ) + nLen;
	nByte = WideCharToMultiByte( CP_UTF8, 0, pszWide, nLength, pszOut, nByte, NULL, NULL );

	strValue.ReleaseBuffer( nLen + nByte );
	delete [] pszWide;
#endif
	ValueToUniform(strValue, strXML);
}
...
#if _MSC_VER >= 1400
	POSITION pos = GetElementIterator();
#else
...
#else
		int nChars = WideCharToMultiByte( CP_UTF8, 0, (LPCWSTR)pByte, nByte, NULL, 0, NULL, NULL );
		WideCharToMultiByte( CP_UTF8, 0, (LPCWSTR)pByte, nByte, strXML.GetBuffer( nChars ), nChars, NULL, NULL );
