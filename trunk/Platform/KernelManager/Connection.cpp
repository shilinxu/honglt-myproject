...
SOCKET CConnection::CreateSocket(Blocking bs)
{
	SOCKET hSocket = socket( PF_INET, SOCK_STREAM, IPPROTO_TCP );
	if ( hSocket == INVALID_SOCKET ) return INVALID_SOCKET;

	if ( ioctlsocket( hSocket, FIONBIO, (DWORD*)&bs ) ) return INVALID_SOCKET;
	return hSocket;
}

DWORD CConnection::TranslateAddr(LPCTSTR pszAddress)
{
	DWORD dwServerIP = inet_addr( CT2CA( pszAddress ) );
	
	if (dwServerIP == INADDR_NONE)
	{
		LPHOSTENT lpHost = gethostbyname( CT2CA( pszAddress ) );
...
{
	if ( m_hSocket == INVALID_SOCKET ) m_hSocket = CreateSocket( bs );
	if ( m_hSocket == INVALID_SOCKET ) return FALSE;
...
#ifdef _DEBUG
	CString strLine;
	strLine.Format( _T("%s[%4i][%i] {%s}\n"), lpszHeader, m_hSocket, nLength, ToHex(pBuffer, min(nLength, TEMP_BUFFER)) );
