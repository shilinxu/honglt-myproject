...
int IRingPacket::GetChNumber(INT& nChNumber) const
{
	nChNumber = ChNumber; return nChNumber;
}
...
void IRingPacket::SetChNumber(INT nChNumber)
{
	ChNumber = nChNumber;
}
...
//////////////////////////////////////////////////////////////////////
// IUserPacket fields access

int IUserPacket::GetUserId(LPTSTR xUserId) const
{
	int nLength = _tcslen( UserId ); nLength = min( nLength, 21 );
	CopyMemory( xUserId, UserId, nLength ); return nLength;
}

BOOL IUserPacket::IsHashUsrCompatible(LPCTSTR xUserId, LPCTSTR xPass)
{
	ASSERT( xUserId != NULL );
	
	IUserPacket* pPacket = new IUserPacket( xUserId, xPass );
	BOOL bResult = _tcsnicmp( MD5Hashing, pPacket->MD5Hashing, 32 ) == 0;
	
	delete pPacket; return bResult;
}
