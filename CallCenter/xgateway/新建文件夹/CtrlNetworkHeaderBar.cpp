...
void CNetworkHeaderBar::Update(CNetworkChild* pChild)
{
	CString strTitle;
	
	if ( pChild != NULL ) pChild->GetHeaderContent( strTitle );
	...
	{
		CString xTitle;
		xTitle.Format( "浏览 %s 的在线用户", strTitle );
		strTitle = xTitle;
	}
	
	if ( m_strTitle.CompareNoCase(strTitle) )
	{
		m_strTitle = strTitle;
