...
void CNetwork::StopThread()
{
	...
	if ( nAttempt == 0 )
	{
		...
		_RPTF0(_CRT_WARN,"WARNING: Terminating CNetwork thread.\n");
		Sleep( 100 );
	}
...
CTransfer* CNetwork::CreateNeighbour(SOCKET hSocket, SOCKADDR_IN* pHost)
{
	CSingleLock pLock( &m_pSection, TRUE );
	
	WSAEventSelect( hSocket, m_pWakeup, FD_READ | FD_WRITE | FD_CLOSE );

	return new CChannel( hSocket, pHost );
}

BOOL CNetwork::RunNeighbour()
{
	CTransfer* pNeighbour = NULL;
...
#endif
	{
		...
		{
			pNeighbour = GetNext( posNext );
		
			if ( ! pNeighbour->OnRun() ) return FALSE;
		}
	}
#ifndef _DEBUG
	catch(_com_error e) 
	{ 
		CString strHost = inet_ntoa( pNeighbour->m_pHost.sin_addr );
		theApp.Message( MSG_ERROR, "ERROR[%s]: %s", strHost, e.ErrorMessage() );
		return FALSE;
	}
	catch(...)
	{
		CString strHost = inet_ntoa( pNeighbour->m_pHost.sin_addr );
		theApp.Message( MSG_ERROR, "ABORT[%s]", strHost );
		return FALSE;
	}
#endif
