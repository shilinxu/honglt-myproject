...
BOOL CAudioSettingsPage::OnInitDialog() 
{
	...
	EnumDevicesBox( CLSID_AudioInputDeviceCategory, &m_wndInput );
	EnumDevicesBox( CLSID_AudioRendererCategory, &m_wndOutput );
	LoadSettings();
...
{
	LoadSettings( &m_wndInput, (LPCTSTR)Settings.Audio.Input );
	LoadSettings( &m_wndOutput, (LPCTSTR)Settings.Audio.Render );
...
{
	EditSettings( &m_wndInput, Settings.Audio.Input );
	EditSettings( &m_wndOutput, Settings.Audio.Render );
...
BOOL CAudioSettingsPage::EditSettings(CComboBox* pCombo, CString& xDevName)
{
	...
	if ( nItem == CB_ERR ) {xDevName.Empty(); return TRUE;}
...
int CAudioSettingsPage::EnumDevicesBox(REFCLSID rclsid, CComboBox* pCombo)
{
	...
	while ( ieMoniker->Next(1, &pMoniker, NULL) == S_OK )
	{
		...
		int nItem = pCombo->AddString( (CString)TheValue.bstrVal );
		pCombo->SetItemDataPtr( nItem, pMoniker );
	}
